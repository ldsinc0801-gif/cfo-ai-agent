...
/**
 * 動画・複数画像で「物理的に別の紙のレシートはすべて別仕訳」とAIに守らせるためのプロンプト。
 * 交通費レシートのように、同金額の別レシートが複数枚あるケースを正しく扱う。
 */
const MULTI_RECEIPT_DEDUP_RULES = `【重要：レシートの枚数を正確に数える】
- 映っている物理的なレシートはすべて別の仕訳として記録してください
- 日付・金額・取引先が同じでも、別の紙であれば別仕訳です（例: 同じ駅の同じ運賃の交通費レシートが2枚あれば2件、3枚あれば3件）
- レシートを合体・要約・省略しないでください。映った枚数 = 仕訳の件数
