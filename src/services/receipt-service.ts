...
export class ReceiptService {
  ...
  async analyzeVideo(videoBuffer: Buffer, mimeType: string, fileName: string, industry?: string, fiscalMonth?: number | null, fiscalYear?: number | null): Promise<ReceiptAnalysis> {
    ...
    const response = await this.ai!.models.generateContent({
      ...
      contents: [{ role: 'user', parts: [
        { inlineData: { mimeType, data: base64 } },
        { text: `この動画に映っている領収書・レシート・請求書をすべて読み取り、それぞれを別の仕訳として生成してください。

${MULTI_RECEIPT_DEDUP_RULES}
...
${prompt}` },
  ...
  async analyzeVideoFrames(frames: { buffer: Buffer; mimeType: string }[], industry?: string, fiscalMonth?: number | null, fiscalYear?: number | null): Promise<ReceiptAnalysis> {
    ...
    const parts = [
      ...
      { text: `これらは領収書・レシートを撮影した画像です（合計${frames.length}枚）。
各画像に写っている領収書・レシートを全て読み取り、それぞれを別の仕訳として生成してください。
...
/**
 * 動画・複数画像での「同じレシートを別フレームで写しているだけ」と
 * 「日付/金額が同じでも別物のレシート」を区別させるためのプロンプト。
 * 交通費レシートのように、同金額の別レシートが複数枚あるケースを正しく扱う。
 */
const MULTI_RECEIPT_DEDUP_RULES = `【重要：レシートの枚数を正確に数える】
- 物理的に別のレシートはすべて別の仕訳として記録してください。日付・金額・取引先が同じでも別仕訳です
  例: 同じ駅で同じ運賃の交通費レシートが2枚あれば2件、3枚あれば3件の仕訳
- 「同じレシートが複数フレームに映っている」と判断してまとめるのは、明確に同一の1枚と確信できる場合のみ:
  - レシート用紙のしわ・印字位置・周囲の手や机・背景まで完全に同じ
  - 角度違いや距離違いで同じ1枚を撮っているだけ
- 以下のいずれかが当てはまるなら別物として扱ってください（迷ったら別物にしてください）:
  - レシートが2枚以上同時に映っている
  - 別のフレームで明らかに別の用紙（紙の輪郭・折れ位置・周囲が違う）
  - レシート番号や発行時刻が記載されており異なる
  - 1枚ずつ手で取り出しながら撮影されている
- 解析結果の notes 配列の先頭に「動画/画像内で確認した領収書: 計N枚」を必ず記載してください
- 1件にまとめた仕訳がある場合は notes に「重複と判断してまとめた件数」を記載してください`;
